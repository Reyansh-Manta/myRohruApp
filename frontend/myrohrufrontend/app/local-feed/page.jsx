...
export default function LocalFeedPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND}/api/v1/posts/category/local-feed?sortBy=${sort}`,
                            ...
                            {posts.map((post) => (
                                ...
                                <div className={styles.container}>
                                    <div style={{display: "flex", justifyContent: "center", alignItems: "center"}} className={styles.incont}>
                                        <img style={{ width: "inherit"}} src={post.image == ""? post.image : "/placeholder.png"} alt="" />
                                    </div>
